...
sass.render({
	...
	if (!error) {
		...
		fs.writeFile("beamsaber.css", cssOutput, printOutput);
	} else {
...
// Build HTML
const htmlOutput = pug.renderFile("Source/beamsaber.pug", options).trim().replace(/\n+/g, "\n");
fs.writeFile("beamsaber.html", `${htmlOutput}\n`, printOutput);
