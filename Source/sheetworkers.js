...
const sheetName = "Masks - A New Generation";
...
const playbookAttributes = [...new Set([].concat(...Object.keys(data.playbook).map(x => Object.keys(data.playbook[x].base))))],
	watchedAttributes = new Set(playbookAttributes),
	...
	translatedNames = [...Object.keys(data.playbook)].reduce((m, keyName) => {
...
/* Pseudo-radios: TODO use this for labels */
...
on("sheet:opened", () => {
	getAttrs(["sheet_type", "changed_attributes", "playbook"], v => {
		/* Make sure sheet_type is never 0 */
		if (!["villain", "story"].includes(v.sheet_type)) setAttr("sheet_type", "hero");
